DB_URL='postgres://postgres:1111@localhost:5433/collaboration?sslmode=disable'
run:
...
mig-file:
	migrate create -ext sql -dir migrations -seq comments

mod:
	go mod init colloboration_service
	go mod tidy
	go mod vendor
tidy:
	go mod tidy
